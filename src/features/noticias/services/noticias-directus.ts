...
export async function getAllNoticiasDirectus() {
...
export async function getNoticiasPaginadasDirectus(
  page: number = 1,
  pageSize: number = 4,
...
export async function getNoticiaBySlugDirectus(slug: string): Promise<Noticia | null> {
  const noticias = await directus.request(
    readItems('noticias', {
  ...
  return {
    ...
    imagen: n.imagen,
    imagenes: n.imagenes, // Agregar este mapeo
...
export async function getNoticiasRelacionadasDirectus(categoria: string, excludeSlug?: string) {
  const filter: any = {
...
export function getPortadaDirectus({ noticia }: any) {
...
export function getImagenesDirectus(noticia: Noticia) {
...
// 7. Categorías únicas
export async function getNoticiasCategoriasDirectus(): Promise<Array<{ id: number; nombre: string }>> {
  const noticias = await directus.request(
    readItems('noticias', {
      fields: ['categoria'],
      limit: -1,
